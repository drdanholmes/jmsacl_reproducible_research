...
The above options suppress the R code in the document (`echo = FALSE`), all R warnings (`warning = FALSE`), and messages (`message = FALSE`), but permits variables to be assigned and tables/figures to be generated. The following template can be used to set these variables globally:
...
When an author is satisfied that their manuscript is ready for submission to JMSACL, they may knit the document a final time. This will generate a finalized LaTeX source file in the same folder as the .Rmd file with the same name but with a .tex file extension. 
There will also be a folder generated and named according to the .Rmd file, in which there will be a subfolder, named figure-latex, that will contain all figures generated from the R code. Proceed with your submission via the Elsevier Editorial Manager site submitting the pdf as your manuscript file and the figures as per usual. Additionally, your .tex file, .bib file, and .csl file must each be submitted as a "LaTeX Source File".
