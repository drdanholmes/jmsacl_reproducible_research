...
# Appendix A: Using the renv package to generate a local library {.unnumbered}
...
To *reconstruct* the the library used to generate the RMarkdown output of this manuscript, one must clone (or download) the [github repository](https://github.com/drdanholmes/jmsacl_reproducible_research) @drdanholmesgithub to their local machine, define it as an R project and the execute `renv::init()` and `renv::restore()`.  This will download all the correct versions of the R packages to a within-project local library. When the RMarkdown file is knit, the local library will be used. 
